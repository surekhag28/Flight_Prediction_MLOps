...
def register_all(
    ...
    Returns:
        {"delay":RegistrationResult, "congestion":}
    """
    ...
    with child_run(
        ...
        mlflow.log_metric("models_promoted", sum([delay_result.promoted]))

        logger.info(
            f"Registration completed, registry_run_id:{run.info.run_id}, delay_promoted:{delay_result.promoted}"
        )

    return {"delay": delay_result}
